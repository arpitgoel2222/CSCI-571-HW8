...
export class Cards extends Component {
  ...
  callguardianAPI() {
    fetch("https://arpitnodehw8.azurewebsites.net/guardian")
      .then(res => {
        return res.json();
      })
      .then(gnews => {
  ...
  changes()
  {
    this.props.callbackFromParent3(this.state.ls);
  }


  callnytAPI() {
    fetch("https://arpitnodehw8.azurewebsites.net/nyt")
      .then(res => {
        return res.json();
      })
      .then(nnews => {
  ...
  render() {
    ...
    {if (this.props.checked === true) {
      return (
        <div>
          <CardShow news={this.state.gnews} ls={this.props.loas}   />
        </div>
      );
      ...
      return (
        <div>
          <NYTCardShow news={this.state.nnews} ls={this.props.loas}  />
        </div>
      );
